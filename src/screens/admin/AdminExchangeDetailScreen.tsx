...
const statusLabels: { [key in ExchangeStatus]: string } = {
  REQUESTED: '접수',
  ACCEPTED: '접수 완료',
  REJECTED: '접수 반려',
  COLLECTING: '상품 회수',
...
export default function AdminExchangeDetailScreen() {
  ...
  const fetchExchangeDetail = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/v1/admin/exchanges/${exchangeId}`,
        {
          headers: {
            Accept: '*/*',
      ...
      const data: ExchangeDetail = await res.json();
      setDetail(data);
    } catch (error) {
      console.error(error);
  ...
  const updateStatus = async () => {
    ...
    try {
      ...
      const query = modalReason.trim()
        ...
        : '';
      const res = await fetch(
        `http://localhost:8080/api/v1/admin/exchanges/${exchangeId}/status/${modalStatus}${query}`,
        {
          method: 'PATCH',
          headers: {
  ...
  return (
    <View style={styles.container}>
      <Header title="교환 상세" showRightIcons={false} showHomeButton={false} />
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoText}>교환 ID: {detail.exchangeId}</Text>
        ...
        <View style={styles.infoBox}>
          <Text style={styles.sectionTitle}>교환 사유</Text>
                ...
                >
                  <Text
                    style={[
                      ...
                      modalStatus === status && styles.statusButtonTextActive,
                    ]}
                  >
                    {statusLabels[status]}
                  </Text>

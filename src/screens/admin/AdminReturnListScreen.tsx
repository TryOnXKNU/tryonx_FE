...
export default function AdminReturnListScreen({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    if (searchText.trim()) {
      ...
      filtered = filtered.filter(item => {
        ...
        const matchesExchangeId = item.returnId
          .toString()
          .includes(lowerSearch);
  ...
  const openStatusModal = (returnId: number, _currentStatus: ReturnStatus) => {
  ...
  const renderStatusButtons = (item: ReturnItem) => (
    <TouchableOpacity
      ...
      onPress={() => openStatusModal(item.returnId, item.status)}
    >
  ...
  const renderItem = ({ item }: { item: ReturnItem }) => (
    <TouchableOpacity
      onPress={() =>
        navigation.navigate('AdminReturnDetail', {
      ...
      <View style={styles.itemContainer}>
        ...
        <View style={styles.info}>
          ...
          <View style={styles.infoGroup}>
            <Text style={styles.groupTitle}>반품 정보</Text>
            <View style={styles.infoRow}>
              <Text style={styles.infoLabel}>반품 ID:</Text>
              <Text style={styles.infoValue}>{item.returnId}</Text>
            </View>
          </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        placeholder="상품명, 반품 ID, 주문 ID, 주문아이템 ID를 검색하세요."
        value={searchText}

...
export default function RecentMembersScreen() {
    ...
    const fetchRecentMembers = async () => {
      try {
        const response = await axios.get<Member[]>(
          'http://localhost:8080/api/v1/admin/members/recent',
          {
            headers: {
        ...
        console.error(error);
        Alert.alert('에러', '신규 회원 목록을 불러오는 데 실패했습니다.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <View style={styles.container}>
      <Header title="신규 회원" showRightIcons={false} hideBackButton={false} />
      <FlatList
        data={members}
        keyExtractor={item => item.memberId.toString()}
        renderItem={renderItem}
        ListEmptyComponent={
          <Text style={styles.emptyText}>신규 회원이 없습니다.</Text>
        }

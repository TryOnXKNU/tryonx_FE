...
export default function ExchangeDetailScreen() {
  const { params } = useRoute<ExchangeDetailRouteProp>();
  const { token } = useAuthStore();
  const [item, setItem] = useState<ExchangeItem | null>(null);
  const [loading, setLoading] = useState(true);

  const BASE_URL = 'http://localhost:8080'; // Android는 http://10.0.2.2:8080로 변경 필요
    ...
    const fetchDetail = async () => {
      try {
        const res = await axios.get(
          `${BASE_URL}/api/v1/exchange/${params.exchangeId}`,
          {
  ...
  return (
    <View style={styles.container}>
      <Header title="교환 상세" showRightIcons={false} />
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>교환 정보</Text>
          ...
          <Text style={styles.value}>{formatDate(item.requestedAt)}</Text>

          {item.processedAt && (
            <>
              <Text style={styles.label}>처리일</Text>
              <Text style={styles.value}>{formatDate(item.processedAt)}</Text>
            </>
          )}
        </View>

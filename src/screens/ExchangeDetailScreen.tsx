// screens/ExchangeDetailScreen.tsx
    ...
    const fetchDetail = async () => {
      try {
        const res = await axios.get(
          `http://localhost:8080/api/v1/exchange/${params.exchangeId}`,
          {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.text}>{formatDate(item.requestedAt)}</Text>

        {item.processedAt && (
          <>
            <Text style={styles.label}>처리일</Text>
            <Text style={styles.text}>{formatDate(item.processedAt)}</Text>
          </>
        )}
